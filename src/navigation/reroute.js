...
export function reroute(pendingPromises = [], eventArguments) {
  ...
  function loadApps() {
    return Promise.resolve().then(() => {
      // gongdf - 加载子应用js，注册钩子函数
      // gongdf - getAppsToLoad()获取需要load的app
      // gongdf - map(toLoadPromise)转换成一个promise数组
      ...
      const mountPromises = getAppsToMount()
        ...
        .map(appToMount => {
          return toBootstrapPromise(appToMount)
            .then(() => unmountAllPromise) // gongdf - 等待其他app unmount完成
  ...
  // gongdf - 完成子app的加载、注册后运行，再去reroute下一个
